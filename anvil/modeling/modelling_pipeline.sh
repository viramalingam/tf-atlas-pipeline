...
if [[ indices_files != '' ]];then
 ...
 echo $(wc -l ${data_dir}/${experiment}_peaks.bed | cut -c1 )
 
 seq 0 $(wc -l ${data_dir}/${experiment}_peaks.bed | cut -c1)> $indices_dir/test_peaks_all_chroms_indices.txt
 
 seq 0 $(wc -l ${data_dir}/${experiment}_background_regions.bed | cut -c1)> $indices_dir/all_peaks_all_chroms_indices.txt
...
predict \\
    ...
    --test_indices $all_peaks_test_chroms_indices_file \\
    --reference-genome $reference_dir/hg38.genome.fa \\
